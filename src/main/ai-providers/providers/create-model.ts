...
export function createModel(providerId: string, modelId: string): LanguageModel {
  ...
  if (providerId === 'openai' && cred.type === 'oauth') {
    ...
    const effectiveModel = modelId.includes('codex') ? modelId : CODEX_DEFAULT_MODEL;
    // OAuth Codex must use Responses API
    return provider.responses(effectiveModel);
  }
  ...
  if (providerId === 'anthropic' && cred.type === 'oauth') {
    ...
    const provider = createAnthropic({ apiKey: 'oauth-placeholder', fetch: wrapper });
    return provider(modelId);
  }

  if (providerId === 'anthropic') {
    const provider = createAnthropic({ apiKey: (cred as CredentialApi).key });
    return provider(modelId);
  }
