...
export const CODEX_DEFAULT_MODEL = 'gpt-4.1-mini';
...
function extractAccountIdFromClaims(claims: Record<string, unknown>): string | undefined {
  return (
    ...
    (claims as { 'https://api.openai.com/auth'?: { chatgpt_account_id?: string } })[
...
function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, '127.0.0.1', () => {
      oauthServer = server;
      resolve({ port: OAUTH_PORT, redirectUri: `http://localhost:${OAUTH_PORT}/auth/callback` });
    });
  });
}
