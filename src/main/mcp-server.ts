...
function buildToolShapes(): ToolShapes {
  ...
  for (const [name, toolDef] of Object.entries(tools)) {
    ...
    shapes[name] = {
      ...
      shape: {
        ...inputSchema.shape,
        workspace: z.string().describe('Workspace slug (run listWorkspaces to see available slugs)'),
      },
    }
  }
...
function buildMcpServer(toolShapes: ToolShapes): McpServer {
  ...
  server.tool('listWorkspaces', 'List all Litho workspaces with their slugs and titles.', {}, async () => {
    const workspaces = await listWorkspaces()
    if (workspaces.length === 0) return { content: [{ type: 'text' as const, text: '(no workspaces)' }] }
